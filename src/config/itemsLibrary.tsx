...
export const itemlib: any = {
  ...
  'letterPaper': {
    name: 'Letter Paper', sell: 1, buy: 10, desc: 'A paper for writing letters.', icon: 'memo', use: function use() {
      ...
      else if (final.startsWith('/fly ') && checkAdmin()) {
        let arg = final.replace('/fly ', '').trim();
        console.log(arg);
        if (arg == '' || !Number(arg)) return;
        coin.addCoins(Number(arg));
      }
      else if (write.startsWith('/give ') && checkAdmin()) {
  ...
  'water': { name: 'Water', sell: 0, buy: 5, desc: 'Fresh and cool water.', icon: 'cup-with-straw' },
  ...
  'fancyPill': { name: 'Fancy Pill', sell: 1000, buy: 50, desc: 'Consume to feel fancy.', icon: 'cocktail-glass' },
  ...
  'richPill': { name: 'Rich Pill', buy: 1000, sell: 15, desc: 'Consume to feel rich and lucky.', icon: 'wine-glass' },
  ...
  'angerPill': { name: 'Anger Pill', sell: 500, buy: 10, desc: 'Consume to violate without feeling guilt.', icon: 'beer-mug' },
  ...
  'chokePill': { name: 'Choke Pill', sell: 500, buy: 20, desc: 'Consume to choke on balls.', icon: 'bubble-tea' },
  ...
  'depression': { name: 'Depression', sell: -5, buy: 1000, desc: 'Half empty? How about JUST Empty? You deserve it.', icon: 'pouring-liquid' },
