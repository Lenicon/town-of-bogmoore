...
export const itemlib: any = {
  ...
  'letterPaper': {
    name: 'Letter Paper', sell: 1, buy: 10, desc: 'A paper for writing letters.', icon: 'memo', use: function use() {
      ...
      let checkmatch = write.toLowerCase().match(/sor(y|rry|rrry|yy|rryyy)?|sori|sowwy|apolog(y|ies|ize)?|my bad/);
      if (checkmatch != null && checkmatch != undefined) {
        inv.addItem('apologyLetter', 1);
        inv.subItem('letterPaper', 1);
      } else {
  ...
  'apologyLetter': { name: 'Apology Letter', sell: -1, desc: 'Letter about how sorry you are.', icon: 'envelope' },

  'trash': { name: 'Useless Trash', sell: -5, desc: 'Just plain garbage. Why does this exist?', icon: 'index-pointing-at-the-viewer' },

  'mysteryGift': {
    name: 'Mystery Gift', sell: 50, desc: "Open it! I wonder what's inside...", icon: 'wrapped-gift', use: function use() {
